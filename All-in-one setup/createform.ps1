...
try {
    ...
    if([string]::IsNullOrEmpty($response.dataSourceGUID)) {
        ...
        Write-ColorOutput Green "Task data source '$dataSourceName' created: $dataSourceSelectOUGuid"
    } else {
        #Get DatasourceGUID
        $dataSourceSelectOUGuid = $response.dataSourceGUID
        Write-ColorOutput Yellow "Task data source '$dataSourceName' already exists: $dataSourceSelectOUGuid"
    }
} catch {
